...
import { createOrder } from '../../../api/itemApi';
...
export default function Deliveryinfo() {
  ...
  useEffect(() => {
    if (!item) {
      console.warn("No item data found, redirecting...");
  ...
  const marketplaceFee = Number((subtotal * 0.03).toFixed(2));
  const deliveryFee = method === 'courier' ? (Number(bid) || 3) : 0;
  const total = Number((subtotal + marketplaceFee + deliveryFee).toFixed(2));
  ...
  const handleSubmit = async () => {
    ...
    const sellerId = item.userId?._id || item.userId || item.owner;

    if (!itemId || !sellerId) {
       setErrors({ submit: "Item or Seller information is missing. Please refresh and try again." });
       return;
    }
    ...
    if (validateForm()) {
      ...
      try {
        ...
        formData.append('courierBid', bid || '0');
        ...
        const errorMsg = err.response?.data?.message || err.message || "Failed to submit order. Please try again.";
        ...
        <div className="flex flex-col md:flex-row gap-5 items-start w-full">
          ...
          <div className="bg-white rounded-[20px] border-[1.5px] border-[#E4E8EF] shadow-[0_8px_40px_rgba(0,0,0,0.08)] flex-1 overflow-hidden">
            <div className="px-7 py-6 pb-5 border-b-[1.5px] border-[#E4E8EF]">
              ...
              {errors.submit && (
                <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm font-bold animate-[fadeIn_0.3s_ease]">
                  ⚠️ {errors.submit}
                </div>
              )}
            </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <select value={room} onChange={e => setRoom(e.target.value)} disabled={!building} className="p-2.5 border-[1.5px] border-[#E4E8EF] rounded-lg bg-[#F8FAFC] text-[13px] text-[#111827] outline-none transition-all focus:border-[#1E3A5F] focus:bg-white cursor-pointer appearance-none disabled:opacity-60">
                  ...
                  <div className="flex flex-col gap-1 sm:col-span-2">
                    ...
                    <select value={timeWindow} onChange={e => setTimeWindow(e.target.value)} className="p-2.5 border-[1.5px] border-[#E4E8EF] rounded-lg bg-[#F8FAFC] text-[13px] text-[#111827] outline-none transition-all focus:border-[#1E3A5F] focus:bg-white cursor-pointer appearance-none">
            ...
            <div className="px-7 py-4 pb-5 border-t-[1.5px] border-[#E4E8EF] flex gap-2.5 items-center">
              ...
              <button 
                ...
                disabled={loading}
                className={`flex-[2] p-3 text-white rounded-xl font-bold text-[13.5px] shadow-lg shadow-[#1e3a5f26] flex items-center justify-center transition-all ${loading ? 'bg-[#94A3B8] cursor-not-allowed' : 'bg-[#1E3A5F] hover:bg-[#162D4A] hover:-translate-y-[1px]'}`}
              >
                {loading ? 'Processing...' : 'Confirm Delivery Info'}
              </button>
            </div>
          </div>
          ...
          <div className="w-full md:w-[290px] bg-white rounded-[20px] border-[1.5px] border-[#E4E8EF] shadow-[0_8px_40px_rgba(0,0,0,0.08)] p-5 md:sticky md:top-6 shrink-0 mt-5 md:mt-0">
            ...
            <div className="flex items-center gap-3 pb-4 border-b-[1.5px] border-[#E4E8EF] mb-3.5">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[13px] font-bold leading-[1.3] mb-0.5 max-w-full truncate">{item.item_name || item.title || 'Selected Item'}</div>
                <div className="text-[11.5px] text-[#6B7280]">Quantity: 1</div>
              </div>
              <div className="text-[14px] font-bold shrink-0 ml-1.5">LKR {subtotal}</div>
            </div>

            <div className="flex justify-between items-center text-[13px] py-1">
              <span className="text-[#6B7280]">Subtotal</span>
              <span className="font-semibold text-[#111827]">LKR {subtotal}</span>
            </div>
            <div className="flex justify-between items-center text-[13px] py-1">
              <span className="text-[#6B7280]">Marketplace Fee (3%)</span>
              <span className="font-semibold text-[#111827]">LKR {marketplaceFee}</span>
            </div>
            <div className="flex justify-between items-center text-[13px] py-1">
              <span className="text-[#6B7280]">Delivery Fee</span>
              <span className="font-bold text-[#10B981]">{method === 'courier' && bid ? `LKR ${parseFloat(bid).toFixed(2)}` : 'FREE'}</span>
            </div>
