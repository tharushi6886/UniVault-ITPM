    ...
    const load = async () => {
      try {
        ...
        if (res && res.data && res.data.items && mounted) {
          // determine current logged-in user id from localStorage
          let storedUser = null;
          ...
          // filter items to only those belonging to the logged-in user
          let itemsFromApi = Array.isArray(res.data.items) ? res.data.items : [];
          if (currentUserId) {
            itemsFromApi = itemsFromApi.filter(it => {
              const ownerId = it.userId || (it.userId && it.userId._id) || '';
              return String(ownerId) === String(currentUserId);
            });
          } else {
            // if no logged-in user, show empty list on "My Items"
            itemsFromApi = [];
          }
          ...
          const mapped = itemsFromApi.map(it => ({
            ...
            brand: it.brand || '',
            cond: it.item_condition || it.cond || '',
            listing: it.item_type || it.listing_type || it.listing || 'sell',
            avail: it.availability_status || it.avail || 'available',
            price: (typeof it.price === 'number') ? it.price : (it.price ? parseFloat(it.price) : 0),
            qty: (typeof it.quantity === 'number') ? it.quantity : (it.quantity ? parseInt(it.quantity, 10) : 0),
            desc: it.description || it.desc || '',
            color: it.colour || it.color || '',
            date: it.createdAt ? new Date(it.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : (it.date || new Date().toLocaleDateString('en-GB')),
            bid: it.bid || null,
            buyNow: it.buyNow || null,
            ...
            {filteredItems.map(item => (
              <div key={item.id} onClick={() => { setSelectedItemId(item.id); setIsPanelOpen(true); }} className="bg-white border-[1.5px] border-[#e4d9f7] rounded-2xl shadow-[0_3px_14px_rgba(46,0,96,0.07)] overflow-hidden transition-all hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(46,0,96,0.13)] cursor-pointer flex flex-col group">
                <div className="h-40 bg-gradient-to-br from-[#f5f3ff] to-[#ede9fe] relative flex items-center justify-center border-b border-[#e4d9f7]">
                  ...
                  {item.image ? (
                    <img src={item.image} alt={item.name} className="absolute inset-0 w-full h-full object-cover z-0" />
                  ) : (
        ...
        <div className="fixed inset-0 z-[200] bg-purple-950/30 backdrop-blur-sm flex justify-end" onClick={() => setIsPanelOpen(false)}>
          <div className="w-full max-w-[600px] bg-white h-full shadow-[-8px_0_40px_rgba(46,0,96,0.18)] flex flex-col transition-transform duration-300 transform translate-x-0" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {selectedItem.image ? (
                  <img src={selectedItem.image} alt={selectedItem.name} className="absolute inset-0 w-full h-full object-cover z-0" />
                ) : (
