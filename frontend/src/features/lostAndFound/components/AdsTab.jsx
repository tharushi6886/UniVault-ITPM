...
export default function AdsTab({
  ...
  const filtered = ADS.filter((a) => {
    ...
    const statusMatch = filterStatus === "all" || mappedStatus === filterStatus;
    ...
    <div className="glass-card p-4">
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <div className="mt-0.5 text-[11px] text-slate-500">Manage every post on the community board</div>
        </div>
              ...
              <tr key={a.id} className="border-b border-indigo-300/10 text-[13px] hover:bg-indigo-500/[0.03] transition">
                ...
                <td className="px-3 py-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <button className="rounded-md border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1.5 text-[11.5px] font-bold text-emerald-600 transition hover:bg-emerald-500/20 flex items-center gap-1.5 focus:scale-95" onClick={() => openArchiveModal(a.title, a.student, "found")}>
                      <span>✓</span> Found
                    </button>
                    <button className="rounded-md border border-amber-500/30 bg-amber-50/80 px-2.5 py-1.5 text-[11.5px] font-bold text-amber-600 shadow-sm transition hover:bg-amber-100 flex items-center gap-1.5 focus:scale-95" onClick={() => {}}>
                      <span>🔔</span> Notify
                    </button>
                    <button className="rounded-md border border-indigo-300/30 bg-indigo-50/50 p-1.5 text-[14px] text-slate-500 transition hover:bg-indigo-100 focus:scale-95" onClick={() => openArchiveModal(a.title, a.student, "archive")} title="Archive">
                      🗃️
                    </button>
                  </div>
                </td>
