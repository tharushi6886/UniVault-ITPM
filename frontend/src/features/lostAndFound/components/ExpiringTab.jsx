...
export default function ExpiringTab({ openNotifyModal, globalSearch }) {
  const expiring = ADS.filter((a) => {
    const term = (globalSearch || "").toLowerCase();
    const matchesSearch =
      a.title.toLowerCase().includes(term) ||
      a.student.toLowerCase().includes(term);
    return a.daysLeft <= 7 && matchesSearch;
  });
      ...
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <div className="mt-0.5 text-[11px] text-slate-500">
            These ads expire within 7 days — notify or extend
          </div>
        </div>
              ...
              <tr key={a.id} className="border-b border-indigo-300/10 text-[13px] hover:bg-slate-50/50 transition">
                ...
                <td className="px-3 py-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <button className="rounded-md border border-amber-500/30 bg-amber-50/80 px-2.5 py-1.5 text-[11.5px] font-bold text-amber-600 shadow-sm transition hover:bg-amber-100 flex items-center gap-1.5 focus:scale-95" onClick={() => openNotifyModal(a.student, a.title)}>
                      <span>🔔</span> Notify
                    </button>
                    <button className="rounded-md border border-indigo-400/30 bg-indigo-50 px-2.5 py-1.5 text-[11.5px] font-bold text-indigo-600 shadow-sm transition hover:bg-indigo-100 flex items-center gap-1.5 focus:scale-95">
                      <span>📅</span> Extend
                    </button>
                    <button className="rounded-md border border-slate-300/50 bg-slate-50 p-1.5 text-[14px] text-slate-500 transition hover:bg-slate-100 focus:scale-95" title="Archive">
                      🗃️
                    </button>
                  </div>
                </td>
